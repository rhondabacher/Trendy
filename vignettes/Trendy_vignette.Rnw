...
The object \Robject{TrendyExampleData} is a simulated data matrix containing 50 rows of genes and 40 columns of samples.
...
The parameter \Robject{trendyOut} are the results from the \Rfunction{trendy} function.
...
the user may specify the order/times via the tVectIn parameter in the \Rfunction{trendy} function.
