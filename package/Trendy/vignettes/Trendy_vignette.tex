...
be specified by the user. The \CRANpkg{segmented} R package is used to fit the segmented regression models.
...
The object \Robject{TrendyExData} is a simulated data matrix containing 50 rows of genes and 40 columns of samples.
...
\definecolor{shadecolor}{rgb}{0.941, 0.941, 0.941}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{res} \hlkwb{<-} \hlkwd{trendy}\hlstd{(}\hlkwc{Data} \hlstd{= TrendyExData,} \hlkwc{Max.K} \hlstd{=} \hlnum{2}\hlstd{)}
...
\Rcode{AdjR2.Cut} parameter in the \Rfunction{topTrendy} function. The \Rfunction{topTrendy} function returns
...
\Robject{res.top$Trend} contains the trend specification of the top genes. The function \Rfunction{trendHeatmap} can be used to display these trends:
...
The parameter \Robject{Trendy.Out} are the results from the \Rfunction{trendy} function.
...
\begin{alltt}
\hlkwd{par}\hlstd{(}\hlkwc{mfrow}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{3}\hlstd{,}\hlnum{2}\hlstd{))}
\hlstd{plot2} \hlkwb{<-} \hlkwd{plotFeature}\hlstd{(}\hlkwc{Data} \hlstd{= TrendyExData,}
...
\end{adjustwidth}\begin{kframe}\begin{alltt}
\hlkwd{print}\hlstd{(res.top}\hlopt{$}\hlstd{Breakpoints[}\hlstr{"g2"}\hlstd{,])} \hlcom{# break points}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlkwd{print}\hlstd{(res.top}\hlopt{$}\hlstd{AdjustedR2[}\hlstr{"g2"}\hlstd{])} \hlcom{# adjusted r squared}
\end{alltt}
\begin{verbatim}
##        g2 
## 0.9710139
\end{verbatim}
\begin{alltt}
\hlkwd{print}\hlstd{(res.top}\hlopt{$}\hlstd{Segments[}\hlstr{"g2"}\hlstd{,])} \hlcom{# fitted slopes of the segments}
\end{alltt}
\begin{verbatim}
## NULL
\end{verbatim}
\begin{alltt}
\hlkwd{print}\hlstd{(res.top}\hlopt{$}\hlstd{Segment.Pvalues[}\hlstr{"g2"}\hlstd{,])} \hlcom{# p value of each the segment}
\end{alltt}
\begin{verbatim}
...
the user may specify the order/times via the T.Vect parameter in the \Rfunction{trendy} function.
...
Genes that have a peak along the time-course will have fitted trend somewhere as "up-down". Genes that are oscillating may have the fitted trend "up-down". To extract a list of such genes we can use the \Rfunction{extractPattern}:
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{Shiny_geneSearchV2.png}
