...
<<eval=TRUE>>=
res <- trendy(Data = TrendyExData, Max.K = 2)
...
<<eval=TRUE, warning=FALSE, fig.width=7, fig.align='center', out.width='1\\textwidth'>>=
par(mfrow=c(3,2))
plot2 <- plotFeature(Data = TrendyExData,
...
plot2 <- plotFeature(TrendyExData,
                    Feature.Names = "g2",
                    Trendy.Out = res)
print(res.top$Breakpoints["g2",]) # break points
print(res.top$AdjustedR2["g2"]) # adjusted r squared
print(res.top$Segments["g2",]) # fitted slopes of the segments
print(res.top$Segment.Pvalues["g2",]) # p value of each the segment
@
...
res2 <- trendy(Data = TrendyExData, T.Vect=t.v, Max.K=2)
