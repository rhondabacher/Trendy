...
# Overview
Trendy is a R package that can be used to perform breakpoint analysis on microarray or RNA-seq expression data with ordered conditions (e.g. time course, spatial course).
...
```{r, cache=TRUE}
res <- trendy(TrendyExData, maxk=2)
...
plot1 <- plotmarker(TrendyExData,listname=names(res.trend$firstup)[1:6],fittedres=res)
```
...
```{r, cache=TRUE, fig.height=7}
plot2 <- plotmarker(TrendyExData,listname=names(res.trend$firstdown)[1:6],
										fittedres=res)
```

To plot the 2 genes in the third group of genes (first no change):
```{r, cache=TRUE, fig.height=3}
plot2 <- plotmarker(TrendyExData,listname=names(res.trend$firstnochange)[1:2],
...
```{r, cache=TRUE}
res2 <- trendy(TrendyExData, t.vect=t.v, maxk=2)
...
```{r, cache=TRUE}
res <- trendy(TrendyExData, maxk=2, saveObject = TRUE, fileName="exampleObject")
```
Then in R run:
