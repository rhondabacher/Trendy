---
title: "Trendy: Breakpoint analysis of time course expression data"
...
# The model
To illustrate Trendy,
here we use time course gene expression data as an example. 
Note Trendy may also be applied to other types of features (e.g. isoform or exon expression) and/or other
...
For each gene, Trendy fits segmented regression models with varying numbers of breakpoints from 1 to $n_k$. In which $n_k$ defaults to 3 but can also
...
Trendy also outputs fitted trend of each gene. For samples between the $j^{th}$ 
...
Download the Trendy package from:

\url{https://github.com/rhonda/Trendy/tree/master/package}
...
The trendy() function can be used to run gene specific segmented regressions.
...
res.top$radj
```
The toptrendy() function may be used to extract top dynamic genes.
By default, toptrendy() will extract genes whose adjusted $R^2$ $r_g$
is greater or equal to 0.5. To change this threshold, a user may specify the
r.cut parameter in toptrendy() function.
res.top\$radj gives $r_g$ of the top dynamic genes, sorted decreasingly
by $r_g$.


By default the trendy() function only consider genes whose mean expression is greater than 10. 
...
```{r, cache=FALSE, warning=FALSE, fig.height=7}
...
library(gplots)
heatmap.2(TrendyExData[names(res.trend$firstup),],trace="none", Rowv=F,Colv=F, 
...
```{r, cache=FALSE, warning=FALSE}
heatmap.2(TrendyExData[names(res.trend$firstdown),],trace="none", Rowv=F,Colv=F, 
					          scale="row", main="top genes (first go down)")
```

To generate expression heatmap of the second group of genes (first no change):
```{r, cache=FALSE, warning=FALSE, fig.height=4}
heatmap.2(TrendyExData[names(res.trend$firstnochange),],trace="none", Rowv=F,Colv=F,
...
```{r, cache=FALSE, fig.height=7}
plot1 <- plotmarker(TrendyExData, Feature.Names = names(res.trend$firstup)[1:6], Seg.Data = res)
```

The input of function plotmarker() requires the expression data and a list of genes of interest.
The parameter fittedres in function plotmarker() takes trendy() fitted results. 
If it is not specified, the function plotmarker() will
run Trendy model on the genes of interest before plotting.
...
```{r, cache=FALSE, fig.height=7}
plot2 <- plotmarker(TrendyExData, Feature.Names = names(res.trend$firstdown)[1:6],
										Seg.Data = res)
```

To plot the 2 genes in the third group of genes (first no change):
```{r, cache=FALSE, fig.height=3}
plot2 <- plotmarker(TrendyExData, Feature.Names = names(res.trend$firstnochange)[1:2],
										Seg.Data = res, Par.Param=c(1,2))
```
...
the user may specify it via the t.vect parameter in trendy() function.
...
```{r, cache=FALSE}
res2 <- trendy(TrendyExData, T.Vect=t.v, Max.K=2)
res.top2 <- toptrendy(res2)
res.trend2 <- trendheatmap(res.top2, Show.Plot=FALSE)
str(res.trend2)
```
...
```{r, cache=FALSE, fig.height=7}
plot1.new <- plotmarker(TrendyExData, T.Vect=t.v,
									Feature.Names = names(res.trend2$firstup)[1:6], Seg.Data = res2)
```



## Extract genes with certain pattern
Genes that have a peak along the time-course will have fitted trend somewhere as "up-down". Genes that are oscillating may have the fitted trend "up-down-up-down".

```{r, cache=FALSE, fig.height=5}
res3 <- trendy(TrendyExData, Max.K=2)
# Genes that peak
pat1 <- extractpattern(res3, Pattern = c("up","down"))
head(pat1)
plotPat1 <- plotmarker(TrendyExData, T.Vect=t.v,
									Feature.Names = pat1$Gene[1:6], Seg.Data = res3)
...
pat3 <- extractpattern(res3, Pattern = c("up","down"), Delay = 15)
head(pat3)
plotPat3 <- plotmarker(TrendyExData, T.Vect=t.v,
									Feature.Names = pat3$Gene[1:6], Seg.Data = res3)
```
...
# Trendy shiny app
The Trendy shiny app requires an RData object output from the trendy() call, which can be obtained by setting saveObject=TRUE.
