...
$$
For each $k$, the segemented regression estimates $k$ breakpoints 
($b^k_{g,1}$, $b^k_{g,2}$, ... $b^k_{g,k})$ between $1$ and $S$.
The segmented regression also estimate $k+2$ $\beta$s. In which $\beta^k_0$
indicates the intercept, and the other $\beta$s indicate slopes for the $k+1$ segements separated by the $k$ breakpoints.
...
at least of one segements having less than $c_{num}$ samples, 
...
res.top$radj
```
The topsegreg() function may be used to exract top dynamic genes.
...
The fitted slopes for the 3 segements are 3.31, 0.06 and -2.97.
