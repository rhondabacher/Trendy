...
# Overview
SegReg is an R package that can be used to perform breakpoint analysis on microarrays or RNA-seq expression data with ordered conditions (e.g. time course, spatial course).
...
The segmented regression also estimates $k+2$ $\beta$s. In which $\beta^k_0$
indicates the intercept, and the other $\beta$s indicate slopes for the $k+1$ segments separated by the $k$ breakpoints.
We denote the $R^2$ for this model as $r^k_g$.

For a given gene, among the models with varying $k$, SegeReg picks the optimal number of breakpoints for this gene by comparing the $R^2$s:
...
\url{https://github.com/rhondabacher/SegReg/tree/master/package}
...
GetNormalizedMat() functions in EBSeq. More details can be found in the EBSeq vignette:
...
The segreg() function is used to run gene specific segmented regressions.
...
By default, topsegreg() will extract genes whose adjusted $R^2$ is greater or equal to 0.5. To change this threshold, a user may specify the
r.cut parameter in topsegreg() function.
res.top\$radj gives $r_{g, \text{adj}}$ of the top dynamic genes, sorted decreasingly
by $r_{g, \text{adj}}$.


By default the segreg() function only consider genes whose mean expression is greater than 10. 
To use another threshold, a user may specify the parameter meancut in the segreg() function.
...
```{r, cache=TRUE, warning=FALSE, fig.height=6}
...
plot2 <- plotmarker(SegRegExData,listfeatures=names(res.trend$firstnochange)[1:2],fittedreg=res,par.param=c(1,2))
```
...
plot1.new <- plotmarker(SegRegExData,t.vect=t.v,listfeatures=names(res.trend2$firstup)[1:6],fittedreg=res2)
```
...
This R package also contains a shiny app, which takes as input the object created while running the segreg() function (the default name of this object is rdata_object_forShiny). 
# ```{r, cache=TRUE}
# data(rdata_object_forShiny)
```
...
\begin{figure}[h!]
\includegraphics[width=1\textwidth]{viz_genes_single.png}
\caption{Vizualize genes individually.}
\end{figure}
...
\begin{figure}[h!]
\includegraphics[width=1\textwidth]{genes_with_pattern.png}
\caption{Extract list of genes having a specific pattern and output a plot and list of genes.}
\end{figure}
