...
#'      rownames(d1) <- c("g1","g2")
#'      colnames(d1) <- paste0("time", 1:100)
#'      seg.all <- trendy(d1)
#'      seg.all <- results(seg.all)
#' 		  top.genes <- topTrendy(seg.all)
#' @export

topTrendy <- function(trendyOutData, adjR2Cut = .5) {
	...
	seg.all.highr <- names(seg.all.radj.sort)[which(seg.all.radj.sort > adjR2Cut)]
	...
	reformat <- lapply(1:7, function(i) { 
