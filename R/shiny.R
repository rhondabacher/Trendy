...
#' @name trendyShiny
#'
#' @importFrom shiny  actionButton column eventReactive eventReactive 
#'    fluidPage fluidRow HTML htmlOutput mainPanel numericInput 
#'    observe observeEvent plotOutput radioButtons reactive 
#'    reactiveValues renderPlot renderPrint renderTable renderText 
#'    runApp selectInput shinyApp sidebarLayout sidebarPanel sliderInput 
#'    stopApp tags titlePanel updateNumericInput updateRadioButtons 
#'    updateSelectInput updateSliderInput wellPanel headerPanel fileInput
#'    conditionalPanel tabsetPanel tabPanel textInput textOutput
#' @import shiny
#' @import utils
#' @import shinyFiles
#' @importFrom magrittr %>%
        ...
        tabsetPanel(
           ...
           column(5,	
              ...
              textInput("pattern", "Enter pattern (separate by 
     				  ...
     				  textInput("rcut", "Only consider genes with an adjusted R squared 
                                  greater than: ", ".5", 
...
server <- function(input, output, session) {
  ...
  ###  
    ...
    names(LIST) <- c("trendyOut", "origData", "tVect", "ToPrint", "TopTrendy")
    return(LIST)
  })
   ...
   observeEvent(input$Submit, {
      ...
      withProgress(message = 'Finding gene patterns:', value = 0, {
        ...
        genes.pass <- Trendy::extractPattern(IN$trendyOut, Pattern = pattern,
                                      adjR2Cut = rcut, Delay = delay)
  ...
  output$genePlot <- renderPlot({
     ...
     plot(IN$tVect, IN$origData[topg,], pch=20, col="#696969", 
      ...
      if(topg %in% names(IN$trendyOut)) {
        ...
        par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 4, 0),
            new = TRUE)
        ...
        par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 7.5, 0),
            new = TRUE)
        plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
