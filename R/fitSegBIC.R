...
fitSegBIC <- function(Data, maxK = 5, tVectIn = NULL, minNumInSeg = 5,
                      pvalCut = .1, numTry = 100, keepFit = FALSE) {
  ...
  bic.lm <- BIC(lmLinear)
	...
	fit.bp.all <- lapply(whichFit, function(j) {
        lmseg.try <- suppressMessages(try(segmented(lmLinear, seg.Z = ~tVectIn,
			            psi = round(seq(1, lastT, length.out = j + 2)[2:(j + 1)]), 
  ...
  allBIC <- vapply(fit.keep, function(i) {BIC(i)}, numeric(1))
	...
	# finally decide if best BP model is better than linear, if not return linear
	if(allBIC[r.choose] >= bic.lm){
		OUT <- list(Trends = lm.id.sign, Segment.Slopes = lm.slp, 
                    Segment.Trends = lm.sign, 
					          Segment.Pvalues = lm.pval, Breakpoints = NA, 
	...
	r.choose.ori <- whichFit[r.choose] # actual k (before remove the NA fitting)
