...
#' @examples 
#'	myTrends <- trendy(TrendyExampleData[1:5,], tVect=1:40)
...
extractPattern <- function(trendyOutData, Pattern = NULL, 
                      adjR2Cut = .5, Delay = 0)  {
  ...
  #get r2adj for each gene
  segdata.radj <- sort(vapply(trendyOutData, function(i) {i$AdjustedR2}, 
    ...
    if(grepl(Pattern, slps) == TRUE) {
      ...
      for(i in 1:length(patstart)) {
          ...
          if(segdata.bks[[genes.pass[j]]][i] >= Delay) {
            ...
            names(brk) <- paste0(rep("BreakPoint", 
                          nchar(Pattern) - 1), 1:(nchar(Pattern) - 1))
            genes[[k]] <- brk
            k = k + 1
