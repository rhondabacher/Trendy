...
#' @examples m1 <- rbind(c(rep(1,50),1:50), c(100:1))
#'      rownames(m1) <- c("g1","g2")
#'      colnames(m1) <- paste0("time", 1:100)
#'      myTrends <- trendy(m1)
...
trendy <- function(Data = NULL, 
  ...
  SummarizedExperiment::assays(Data)[["Counts"]] <- Trendy::getCounts(Data)[featureNames, ]
  ...
  if (length(featureNames) == 1) {
    ...
    Data.MeanFiltered <- Trendy::getCounts(Data)[which(rowMeans(Trendy::getCounts(Data)) >= meanCut),]
  ...
  if (NSample < (maxK + 1) * minNumInSeg) {
    	...
    	set to", maxK)}
  if (maxK < 1) {stop("Invalid value for maxK. Adjust minNumInSeg setting
  ...
  if (forceRsq == TRUE) {	
    segAll <- BiocParallel::bplapply(X = seq_len(nrow(Data.MeanFiltered)), 
                      function(X) {
                        fitSegRsq(Data = Data.MeanFiltered[X,],
                                  tVectIn = tVectIn,  maxK = maxK, 
                                  minNumInSeg = minNumInSeg, 
                                  pvalCut = pvalCut, 
                                  cutDiff = cutDiff,
